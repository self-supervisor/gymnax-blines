...
class Visualizer(object):
    def __init__(self, env, env_params, state_seq, reward_seq=None):
        ...
        if env.name not in [
            ...
            self.interval = 100  # * 10
        else:
            self.interval = 50  # * 10

    def animate(
        self, save_fname: Optional[str] = "test.gif", view: bool = False,
    ):
        """Anim for 2D fct - x (#steps, #pop, 2) & fitness (#steps, #pop)"""
        ani = animation.FuncAnimation(
            self.fig,
    ...
    def init(self):
        ...
        if self.env.name in [
            ...
            # Animations have to use older gym version and pyglet!
            assert gym.__version__ == "0.19.0"
            self.im = init_gym(self.ax, self.env, self.state_seq[0], self.env_params)
        elif self.env.name == "Catch-bsuite":
            self.im = init_catch(self.ax, self.env, self.state_seq[0], self.env_params)
        elif self.env.name in [
            "Asterix-MinAtar",
            "Breakout-MinAtar",
            "Freeway-MinAtar",
            ...
            self.im = init_circle(self.ax, self.env, self.state_seq[0], self.env_params)
        elif self.env.name in ["MetaMaze-misc", "FourRooms-misc"]:
            self.im = init_maze(self.ax, self.env, self.state_seq[0], self.env_params)
        self.fig.tight_layout(rect=[0.02, 0.03, 1.0, 0.95])
